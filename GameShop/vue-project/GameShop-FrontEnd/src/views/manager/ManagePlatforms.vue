<!-- File: src/views/manager/ManagePlatforms.vue -->
...
<template>
  <v-container>
    ...
    <v-list>
      ...
      >
        <v-list-item-content>
          <v-list-item-title>{{ platform.platformName }}</v-list-item-title>
        </v-list-item-content>
        <v-list-item-icon>
    ...
    <v-dialog v-model="showEditDialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-text-field label="Platform Name" v-model="selectedPlatform.platformName"></v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default defineComponent({
  ...
  setup() {
    ...
    const platformToDeleteId = ref(null);
    ...
    const createPlatform = async () => {
      try {
        ...
        alert('Failed to create platform. Please check the input and try again.');
      }
    };
    ...
    const updatePlatform = async () => {
      ...
      try {
        await axios.put(`http://localhost:8080/platforms/${selectedPlatform.value.platformId}`, {
    ...
    const deletePlatform = async () => {
      ...
      try {
        await axios.delete(`http://localhost:8080/platforms/${platformToDeleteId.value}`);
        showDeleteDialog.value = false;
        platformToDeleteId.value = null;
        deleteError.value = '';
        await fetchPlatforms();
      } catch (error) {
    ...
    const sortedPlatforms = computed(() => {
      return platforms.value.slice().sort((a, b) => a.platformName.localeCompare(b.platformName));
    });
